...
const ms = require("ms")
     module.exports = {
        ...
        if (currentCooldown === time) return message.channel.send(`Slowmode is already set to ${args[0]}`).then(m => m.delete({ timeout: 5000 }));
        ...
        message.channel.setRateLimitPerUser(args[0], reason).then(m => m.send(embed)).then(m => m.delete({ timeout: 5000}));
